...
interface Props {
  ...
  setShortcuts: (shortcuts: { colorKeys: { [key: string]: string }; sizeKeys: { [key: string]: number } }) => void;
}
...
const Toolbar: React.FC<Props> = ({ setColor, setBrushSize, setTool, shortcuts, setShortcuts }) => {
  ...
  const handleAddColorShortcut = () => {
    if (newColorKey && newColorValue) {
      setShortcuts((prev: { colorKeys: { [key: string]: string }; sizeKeys: { [key: string]: number } }) => ({
        ...prev,
        colorKeys: { ...prev.colorKeys, [newColorKey]: newColorValue }
      }));
      setNewColorKey('');
      setNewColorValue('#000000');
    } else {
      console.error('Invalid color key or value');
    }
  };

  const handleAddSizeShortcut = () => {
    if (newSizeKey && !isNaN(newSizeValue)) {
      setShortcuts((prev: { colorKeys: { [key: string]: string }; sizeKeys: { [key: string]: number } }) => ({
        ...prev,
        sizeKeys: { ...prev.sizeKeys, [newSizeKey]: newSizeValue }
      }));
      setNewSizeKey('');
    } else {
      console.error('Invalid size key or value');
    }
  };
